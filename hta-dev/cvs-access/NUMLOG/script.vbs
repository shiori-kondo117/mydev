'//*********************************************************
'//* @FileName		= script.vbs
'//* @Description  	= 番号採番ログ検索
'//* @Version		= V0.2
'//* @History		= 2008/04/17,DBIクラスを使用する改修,
'//*********************************************************
...
Private Function GetDeleteInfo(byVal intFolderNo)
	Dim strHTML
	Dim strDiscID
	Dim objArr
	...
	strSQL = "SELECT ディスクＩＤ,ボリューム番号,番組名,タイトル名,録画日付,録画時間,削除フラグ,編集フラグ  FROM T_録画記録 where ディスクＩＤ = '" & strDiscID & "'"
...
Private Function makeRecordsetToHTML(byref p_oRs)
	Dim oField
	Dim sHTML

	sHTML = "<table border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbCrLf
	sHTML = sHTML & _
		"<tr>" & vbCrLf
	For Each oField in p_oRs.Fields
	Next
End Function
