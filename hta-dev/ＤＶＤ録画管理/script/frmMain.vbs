...
'//*********************************************************
'// @Procedure cmdExecSqlCommad_Click
'//*********************************************************
Private Sub cmdExecSqlCommad_Click( _
	byref p_objForm _
	)
	'-- var Object
	Dim objConnection
	...
	If p_objForm.Value = "" Then Msgbox "sql command nothing!!":Exit Sub

	Set objConnection = CreateObject("ADODB.Connection")
	Set objRecordSet = CreateObject("ADODB.Recordset")

	strConnStr = "File Name=../conf/default.udl"

	objConnection.Open strConnStr, "", ""

	strSQL = f_txa_sqlcommand.value
...
'--		RecCnt = RecCnt + 1
	...
	document.getElementById("RESULT_LIST").InnerHTML = strHTML
...
'//	strSQL = "select * from T_ディスク番号マスタ"

'//	set objRecordSet2 = objConnection.Execute(strSQL)
'//	objRecordset2.Open strSQL, _
'//	          objConnection, adOpenStatic, adLockOptimistic, adCmdText

'//	strDvdNo=objRecordset2.Fields.Item("ＤＶＤ番号") + 1
	strDvdNo = makeDiscID(DVD_NO.value)

'//	msgbox "DvdNo=" & strDvdNo
	...
	strSQL = "insert into T_録画記録(" & _
		"ディスクＩＤ,ボリューム番号,番組名,タイトル名,録画日付,録画時間,録画モード,コピーワンス,削除フラグ,削除日付,編集フラグ,メモ) " & _
		"values('" & _
		strDvdNo & "', " & _
		VOLUME_NO.value & ", '" & _
		BANGUMI_NAME.value & "', '" & _
		TITLE_NAME.value & "', '" & _
		strRecDate & "', '" & _
		strRecTime & "', '" & _
		REC_MODE.value & "', " & _
		COPYONCE.checked & ", " & _
		DELETE_FLAG.checked & ", " & _
		"0" & ", " & _
		F_EDIT_FLAG.checked & ",'" & _
		F_RECORDER_TYPE.value & "-" & _
		F_MEDIA_TYPE.value & "-" & _
		F_DEST_DISCNO.value & vbCrLf & F_MEMO.value & "')"

	DEBUGLOG strSQL

	set objRecordSet = objConnection.Execute(strSQL)

'//	strSQl = "update T_ディスク番号マスタ set ＤＶＤ番号 = " & strDvdNo
...
'--	Call MakeListBox("ConditionList.txt", KEY_CONDITION)
	f_txa_sqlcommand.value = "SELECT top 10 録画記録番号,ディスクＩＤ,番組名,タイトル名,録画日付 FROM T_録画記録;"
End Sub
